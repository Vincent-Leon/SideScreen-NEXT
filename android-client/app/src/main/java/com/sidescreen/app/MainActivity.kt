...
class MainActivity : AppCompatActivity() {
    ...
    // Set true between user tapping CONNECT and the streamClient handshake
    // resolving (one way or the other). While true, no checklist socket probe
    // may run — even an in-flight `updateChecklist` coroutine that already
    // passed `removeCallbacks` would otherwise open a socket and tickle
    // Mac fork's "single client" eviction, killing our streaming connection.
    @Volatile
    ...
    private fun setupUI() {
        binding.connectButton.setOnClickListener {
            ...
            // Stop checklist probes synchronously BEFORE connect launches.
            // Mac fork's "single client" policy shuts down any prior connection
            // when a new one arrives, so a checkServerRunning() probe firing
            // mid-handshake will kill our streaming socket → first read EOFs.
            // The `isConnecting` flag also gates any *already-launched* probe
            // coroutine that hasn't yet reached its socket open.
            isConnecting = true
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                streamClient?.onConnectionStatus = { connected ->
                    runOnUiThread {
                        // Update connection state flag. The connect attempt
                        // resolved (one way or the other), so checklist
                        // probes can run freely again unless we hand off to
    ...
    private fun updateChecklist() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            // Double-check connection state before socket test. isConnecting
            // covers the window between user tapping CONNECT and stream
            // socket fully established — a probe firing in that window kills
            // the streaming socket via Mac fork's single-client policy.
            if (isConnected || isConnecting) return@launch
            ...
            runOnUiThread {
                ...
                if (isConnected || isConnecting) return@runOnUiThread
    ...
    private fun checkServerRunning(
        ...
        // Last-chance guard: bail before opening any socket if either flag
        // flipped between the caller's check and now. Mac fork's "single
        // client" policy means even a 50ms probe socket opening here will
        // evict an in-progress streaming socket.
